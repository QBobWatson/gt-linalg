...
new Demo({
    ...
    mathbox.select(".axes").set("visible", false);
    ...
    this.labeledVectors(vectors, colors, ['', '', ''], {
        zeroPoints: true,
    });
    ...
    var surfaceColor = new THREE.Color(0.8, 0, 0);
    ...
    if(paramsQS.indep)
        this.label.innerHTML =
            'The three vectors are <strong><span id="span-type">linearly independent</span></strong>'
            + "<br><br>[Drag the vector heads with the mouse to move them]";
    else
        this.label.innerHTML =
        'The span of the vectors is <strong><span id="span-type">space</span></strong>'
	+ '<br><br>The orthogonal complement is '
